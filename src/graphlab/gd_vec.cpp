...
struct vertex_data {
	...
	vertex_data() : 
		role(E), nupdates(0), x(0), y(0)
	{  }
	
		
	vertex_data(std::vector<double>& v) : 
		role(vertex_data::Y), nupdates(0) , x(v.size()), y(v)
	{	
	...
	vertex_data(vertex_role_type role) : 
		role(role), nupdates(0), x(0), y(0)
	{	}
	
	vertex_data(const vertex_data &other) : 
		role(other.role), nupdates(other.nupdates) , x(other.x), y(other.y)
	{	}
	
	/** \brief Save the vertex data to a binary archive */
	void save(graphlab::oarchive& arc) const 
	{
		arc << nupdates << role << (unsigned int)x.size();
		for(unsigned int i=0;i<x.size();i++)
		{
			arc << x[i];
			if(role == X || role == Y)
...
public:
	...
	gather_type gather(icontext_type& context, const vertex_type& vertex, edge_type& edge) const
	{
		...
		//if(other_vdata.x.size()==0)
			//std::cout << "here " << (int)vertex.id() << ":" << roleToChar(vertex.data().role)
			//	<< " " << (int)get_other_vertex(edge, vertex).id() <<":" << roleToChar(other_vdata.role) << std::endl;
	...
	void apply(icontext_type& context, vertex_type& vertex,
		...
		vdata.x.resize(sum.dot.size(),0);
		
		if(vdata.role == vertex_data::X)
		{
			//if(vdata.nupdates==0)//first visit
			//{
			//	for(unsigned int i=0;i< vdata.x.size();i++)
			//	{
			//		vdata.x[i] = ((rand()%256)/256.0);
			//	}
			//}
			//else
			{
				//std::cout << (int)vertex.id() << std::endl;
				for(unsigned int i=0;i< vdata.x.size();i++)
				{
					if(LAMBDA != 0) //TODO: L2, L1 norm
						vdata.x[i] = soft_thereshold(vdata.x[i] - GAMMA*(sum.dot[i]),LAMBDA);
					else
						vdata.x[i] = vdata.x[i] - GAMMA*(sum.dot[i]);
				}
			}
		}
		else
		{
			//if(vdata.x == NULL)
			//	vdata.x = new double [vdata.x_len];
				...
				const vertex_type& vertex) const 
	{
		...
		if(vdata.role == vertex_data::Y)
		{
			...
			if( sqrt(error/norm) < DIFF_THRESHOLD 
...
inline bool graph_edge_vertexPQ_loader(graph_type& graph, 
	...
	if(hasEnding(filename,".d"))
	{
		...
		graph.add_edge(getID(vertex_data::P, id2), getID(vertex_data::Y, id1), edge_data(value,group_id));	
		graph.add_edge(getID(vertex_data::Y, id1), getID(vertex_data::Q, id2), edge_data(value,group_id));	
	...
	else if(hasEnding(filename, ".v"))
	{
		...
		graph.add_edge(getID(vertex_data::X, id2), getID(vertex_data::P, id1), edge_data(value,group_id));	
		graph.add_edge(getID(vertex_data::Q, id1), getID(vertex_data::X, id2), edge_data(value,group_id));	
	...
	public graphlab::IS_POD_TYPE 
{
	...
	static void finalize(icontext_type& context, const error_aggregator& agg) 
	{
		...
		context.cout() << "Time = " << context.elapsed_seconds() << "s\tNorm(AX - Y)/Norm(Y) = " << delta / xnorm << std::endl;
		//if(error < 1e-10)
		//	context.stop();
	}
