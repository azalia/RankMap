	...
	public graphlab::IS_POD_TYPE 
{
	...
	{
		const double error = std::sqrt(agg.error);
		context.cout() << "Time = " << context.elapsed_seconds() << "s\tNorm(x - oldX) = " << error << std::endl;
		if(error < 1e-10)
			context.stop();
	}
...
size_t factor_vertex_program::MAX_UPDATES = 100;
double factor_vertex_program::GAMMA = 0.01;
...
int main(int argc, char** argv) 
{
	...
	int interval = 1; // sec for aggregate error
